	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Seja Bem Vinda(o) @${num.split('@')[0]}\nSem Crocodilagem no(a)*${mdata.subject}*`
				...
				teks = `Mais um croco saiu🐊🐊 @${num.split('@')[0]}👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
			...
			case 'gtts':
                ...
                case 'idiomas':
					...
					break
			    case 'regras':
					reply("*REGRAS PARA UM BOM RELACIONAMENTO NO GRUPO* ♥🥰🙌🏻\r\n*Proibidos:*\r\n🚫 Palavrões (1° Aviso, 2° Ban temporário por 1 hora de tempo, 3° Ban Definitivo.\r\n(Incluindo Palavras escritas, Stickers ou Imagens)\r\n🚫 Links de Grupos ou Rede Socias sem prévio aviso aos Adms (Aviso Imediato e pedido de apagar a mídia, caso seja negado, ban definitivo)\r\n🚫 Conteúdo Sexual (Real, Hentai, Imagem, Video, Etc...) Ban Imediato e Definitivo\r\n🚫 Desrespeito Aos Membros e Administração\r\n1° Aviso, 2° Ban Temporário, 3° Ban Definitivo\r\n🚫 Muito tempo Ghost (Inativo) sem explicação aos Adms, terá Remoção decretada, sem volta.\r\n🚫 Nao  mandar trava (se mandar ban imediato)\r\n🚫Se for fazer grupo fora da arena com membros da arena, avise-nos para não levar ban, tenha capacidade e recrute seus próprios membros (Regras quebrada, Ban definitivo com bombardeio de travas no Pv, achou ruim, vem reclamar no meu pv)\r\n*Permitido:*\r\n✅ Mandar todo o conteúdo Naruto/Boruto\r\n✅ Troca de personagem apenas 2 vezes\r\n✅ Falar sobre assuntos variáveis, vamos fazer amigos 😇\r\n✅ Indicar amigos para ser add aqui\r\n*Mande Um OK Para Sabermos Que Você Chegou Até Aqui e Leu Todas* \r\nDivirtam-Se ♥♥♥\r\nDúvidas❓\r\nPV Dos Adms\r\nKakashi\r\nTemari\r\nNaruto\r\nIzumi\r\nKonan\r\nSuigetsu\r\nYota\r\nPain\r\n🥰✌🏻♥👊🏻 #TMJ\r\nObs: Se possível, favoritem essa Mensagem, para sempre que houver dúvidas, consultá-las!\r\n")
					break
		     	case 'bomdia':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/7VL9cFf.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Bom dia, vcs sao d+ ❤️'})
					break
				case 'boatarde':
					memein = await kagApi.memeindo()
				...
				case 'meme':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'memekkkk'})
					break
				case 'memeindo':
					memein = await kagApi.memeindo()
				...
				case 'prefixo':
					...
					prefix = args[0]
					reply(`O PREFIXO AGORA E : ${prefix}`)
					break
				case 'loli':
                ...
                case 'chamada2':
                 ...
                 case 'chamada3':
					...
					break
				case 'clearall':
					if (!isOwner) return reply('O DONO DO BOT E O PAIN NAO VC 🐊👺')
					anu = await client.chats.all()
					client.setMaxListeners(25)
					for (let _ of anu) {
						client.deleteChat(_.jid)
					}
					reply('ta tudo deletado:)')
					break
				case 'bc':
					if (!isOwner) return reply('O DONO DO BOT E O PAIN NAO VC 🐊👺')
					if (args.length < 1) return reply('.......')
				...
				case 'add':
					...
					if (args[0].startsWith('08')) return reply('pronto ja ta adicionado')
					try {
               ...
               case 'sair':
				...
				case 'imagem':
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'ai tua foto comedia'})
						fs.unlinkSync(ran)
					})
					break
				case 'simi':
